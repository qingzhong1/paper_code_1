...
import argparse
import wandb
wandb.init(project="robert_mosie_mynet_50_16_model_mae_new",name='robert_wandb_332333_robert_all')
...
from my_model_mosie import Roberta, WEIGHTS_NAME, CONFIG_NAME,length_mask,Roberta_transformer_herical,convert_linear_layer_to_lora,only_optimize_lora_parameters
...
path_train='/qingzhong/paper/data/mosie/train.pkl'
path_dev='/qingzhong/paper/data/mosie/dev.pkl'
path_test='/qingzhong/paper/data/mosie/test.pkl'
# 单GPU
...
def main(i):
    ...
    ## Required parameters
    parser.add_argument("--data_dir", default='/qingzhong/paper/data/mosie', type=str,
                        help="The input data dir. Should contain the .tsv files (or other data files) for the task.")
    parser.add_argument("--bert_model", default='/qingzhong/paper/pretrain/roberta', type=str,
    ...
    parser.add_argument("--output_dir",
                        default='/qingzhong/mul/paper/output', type=str,
    ...
    parser.add_argument('--actor_lora_dim',type=int,default=8)
    parser.add_argument('--use_bit',type=int,default=False)
    ...
    tokenizer=RobertaTokenizer.from_pretrained('/qingzhong/paper/pretrain/roberta',do_lower_case=args.do_lower_case)
    train_examples = None
    ...
    ##############################################################################################################
    ###model=Roberta(output_size=100,audio_size=47,vision_size=74)
    model=Roberta_transformer_herical(output_size=768,audio_size=35,vision_size=74)
    actor_lora_module_name='attention'
    # for name, module in model.named_modules():
    #     if actor_lora_module_name in name and isinstance(module, torch.nn.Linear):
    #         print(name,type(module))
    # breakpoint()
    # LoRA
    # if args.actor_lora_dim > 0:
    #     model = convert_linear_layer_to_lora(
    #         model, actor_lora_module_name,
    #         args.actor_lora_dim)
    ...
    # # for name,param in model.named_parameters():
    # #     if 'bias' in name or 'la' in name or 'lv' in name:
    # #         param.requires_grad=True
    #     print(name,param.requires_grad)
    # for name,param in model.named_parameters():
    #     print(name,param.requires_grad)
    #     if 'l1.encoder.layer' not in name and 'la' in name or 'lv' in name or 'tranformer_VA' in name or 'tranformer_Main' in name or 'classifier' in name:
    #         param.requires_grad=True
    #     print(name,param.requires_grad)
    '''for name, param in model.named_p
        for name, param in model.named_parameters():
        print(name)
    ...
    train_audio_length=torch.tensor(train_audio_length)
    valid_audio, valid_audio_length = A_V_data(path_dev, args.max_seq_length)
    valid_audio_length=torch.tensor(valid_audio_length)
    valid_audio = torch.tensor(valid_audio)
    test_audio, test_audio_length = A_V_data(path_test, args.max_seq_length)
    test_audio_length=torch.tensor(test_audio_length)
    test_audio = torch.tensor(test_audio)
    train_vision, train_vision_length = A_V_data(path_train, args.max_seq_length, mode='vision')
    train_vision_length=torch.tensor(train_vision_length)
    valid_vision, valid_vision_length = A_V_data(path_dev, args.max_seq_length, mode='vision')
    valid_vision_length=torch.tensor(valid_vision_length)
    test_vision, test_vision_length = A_V_data(path_test, args.max_seq_length, mode='vision')
    test_vision_length=torch.tensor(test_vision_length)
    ...
    if args.do_train:
        ...
        train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_train_audio, all_train_vision,
                                   all_label_ids,train_audio_length,train_vision_length)
        ...
        eval_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_valid_audio, all_valid_vision,
                                  all_label_ids,valid_audio_length,valid_vision_length)
        ...
        test_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_test_audio,
                                  all_test_vision,test_audio_length,test_vision_length)
        # Run prediction for full data
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            with tqdm(train_dataloader, desc="Iteration") as tepoch:
                for step, batch in enumerate(tepoch):
                    ...
                    input_ids, input_mask, segment_ids, all_train_audio, all_train_vision, label_ids,all_train_audio_mask,all_train_vision_mask = batch
                    loss, logits = model(input_ids, all_train_audio, all_train_vision, segment_ids, input_mask,label_ids,all_train_audio_mask,all_train_vision_mask)
                    ct = MSELoss()
                    loss = ct(torch.squeeze(logits), label_ids)
                    if n_gpu > 1:
            ...
            for step,(input_ids, input_mask, segment_ids, all_valid_audio, all_valid_vision, label_ids,all_valid_audio_mask,all_valid_vision_mask) in enumerate(tqdm(
                    eval_dataloader,
                    desc="Evaluating")):
                ...
                all_valid_audio_mask=all_valid_audio_mask.to(device)
                all_valid_vision_mask=all_valid_vision_mask.to(device)
                with torch.no_grad():
                    tmp_eval_loss, logits = model(input_ids, all_valid_audio, all_valid_vision, segment_ids, input_mask,
                                                  label_ids,all_valid_audio_mask,all_valid_vision_mask)
            ...
            with torch.no_grad():
                for input_ids, input_mask, segment_ids, label_ids, all_test_audio, all_test_vision,all_test_audio_mask,all_test_vision_mask in tqdm(
                        test_dataloader,
                        desc="Evaluating"):
                    input_ids = input_ids.to(device)
                    input_mask = input_mask.to(device)
                    ...
                    with torch.no_grad():
                        tmp_test_loss, logits = model(input_ids, all_test_audio, all_test_vision, segment_ids,
                                                      input_mask, label_ids,all_test_audio_mask,all_test_vision_mask)

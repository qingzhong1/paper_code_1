...
def main(i):
    ...
    if args.do_test:
        ...
        all_segment_ids = torch.tensor([f.segment_ids for f in test_features], dtype=torch.long)
        all_label_ids = torch.tensor([f.label_id for f in test_features], dtype=torch.float32)
        test_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_test_audio,all_test_vision)
        # Run prediction for full data
        test_sampler = SequentialSampler(test_data)
        test_dataloader = DataLoader(test_data, sampler=test_sampler, batch_size=args.test_batch_size)
        '''model = BertForSequenceClassification_three(hidden_dropout_prob=0.1,hidden_size=768,num_labels=1,fusion_dim=5)
        ...
        fusion_attention_list = []
        with torch.no_grad():
            for input_ids, input_mask, segment_ids, label_ids, all_test_audio,all_test_vision in tqdm(test_dataloader,
                                                                                      desc="Evaluating"):
                input_ids = input_ids.to(device)
                input_mask = input_mask.to(device)
                segment_ids = segment_ids.to(device)
                ...
                all_test_vision=all_test_vision.to(device)
                with torch.no_grad():
                    tmp_test_loss, logits = model(input_ids, all_test_audio,all_test_vision, segment_ids, input_mask, label_ids)
                    logits, text_attention, fusion_attention = model(input_ids, all_test_audio,all_test_vision, segment_ids, input_mask)

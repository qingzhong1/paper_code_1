...
import argparse
import wandb
wandb.init(project="bert_mosi_mynet_50_16_model",name='bert_wandb_50_16_2')
...
from my_model import WEIGHTS_NAME, CONFIG_NAME,length_mask,BERT_E
...
def main(i):
    ...
    tokenizer=BertTokenizer.from_pretrained('D:/深度学习/bert_pretraining/',do_lower_case=args.do_lower_case)
    train_examples = None
    ...
    ##############################################################################################################
    model=BERT_E(output_size=100,audio_size=47,vision_size=74)
    ##############################################################################################################
    model.to(device)

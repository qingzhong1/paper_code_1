...
class DIVEncoder(nn.Module):
    ...
    def __init__(self, in_size, out_size, prj_type='linear', use_disc=True,
    ...
    def forward(self, input_l, input_o, lengths, mask):
        ...
        if self.prj_type == 'linear':
            ...
            enc_o = self.encode_o(avg_o)  # 编码层是一层的双向rnn
        ...
        # enc_l,enc_o为2维[2*batch,embed_size],disc_out和disc_labels为1维[2*batch],[2*batch]
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def forward(self, x, x_k=None, x_v=None, ctr_vec=None, lengths=None, mode='l2o'):
        ...
        def get_mask(batch_size, len1, len2, lengths=None, lang_proj=False):
            ...
            bool_mask1 = torch.BoolTensor(batch_size, len1, len2)
            bool_mask2 = torch.BoolTensor(batch_size, len1, len2)
...
if __name__ == '__main__':
    ...
    h=torch.tensor(torch.rand(2,300))
    lengths=[ 20 for i in range(2)]
    lengths=torch.tensor(lengths)
    output1=encoder(x,x,h,h,lengths)

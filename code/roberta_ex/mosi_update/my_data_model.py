...
from my_model import BertForSequenceClassification, WEIGHTS_NAME, CONFIG_NAME
...
def main(i):
    ...
    ##############################################################################################################
    model = BertForSequenceClassification(hidden_dropout_prob=0.1,hidden_size=768,num_labels=1,fusion_dim=47)
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            with tqdm(train_dataloader, desc="Iteration") as tepoch:
                for step, batch in enumerate(tepoch):
                    ...
                    loss, logits = model(input_ids, all_train_audio, segment_ids, input_mask, label_ids)
                    if n_gpu > 1:
            ...
            for input_ids, input_mask, segment_ids, all_valid_audio, label_ids in tqdm(eval_dataloader,
                                                                                       desc="Evaluating"):
                ...
                with torch.no_grad():
                    tmp_eval_loss, logits = model(input_ids, all_valid_audio, segment_ids, input_mask, label_ids)
    ...
    if args.do_test:
        ...
        model = BertForSequenceClassification(hidden_dropout_prob=0.1,hidden_size=768,num_labels=1)
        ...
        with torch.no_grad():
            for input_ids, input_mask, segment_ids, label_ids, all_test_audio in tqdm(test_dataloader,
                                                                                      desc="Evaluating"):
                input_ids = input_ids.to(device)
                input_mask = input_mask.to(device)
                segment_ids = segment_ids.to(device)
                ...
                with torch.no_grad():
                    tmp_test_loss, logits = model(input_ids, all_test_audio, segment_ids, input_mask, label_ids)

...
def main(i):
    ...
    ## Required parameters
    parser.add_argument("--data_dir", default='D:\\A_L_V\\Cross-Modal-BERT-master\\data\\text', type=str,
    ...
    parser.add_argument("--output_dir", default='D:\\A_L_V\\Cross-Modal-BERT-master\\Cross-Modal-BERT-master\\CM-BERT_output', type=str,
    ...
    nb_tr_steps = 0
    tr_loss = 0
    train_audio,valid_audio,test_audio= pickle.load(open('D:\\A_L_V\\Cross-Modal-BERT-master\\data\\audio\\MOSI_cmu_audio_CLS.pickle','rb'))
    max_acc = 0
    min_loss = 100
    if args.do_train:
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            with tqdm(train_dataloader, desc="Iteration") as tepoch:
                for step, batch in enumerate(tepoch):
                    ...
                    nb_tr_steps += 1
                    tepoch.set_postfix(
                        {'Batch': '{:3d}'.format(step), 'Train Loss': ' {:5.4f}'.format(loss),'acc':'{:5.4f}'.format(train_accuracy_rate),'acc_num':'{:5.4f}'.format(tmp_train_accuracy)})
...
if __name__ == '__main__':
    ...
    acc7_list = []
    for i in range(5):
        os.system('D:\\A_L_V\\multimodal-deep-learning-main\\Cross-Modal-BERT-master\\Cross-Modal-BERT-master\\CM-BERT_output')
        results = main(i)

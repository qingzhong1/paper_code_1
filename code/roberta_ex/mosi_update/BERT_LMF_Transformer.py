...
from my_model import Bert_LMF_Tranformer, WEIGHTS_NAME, CONFIG_NAME,length_mask
...
def main(i):
    ...
    ##############################################################################################################
    model=Bert_LMF_Tranformer(hidden_dropout_prob=0.1,hidden_size=768,num_labels=1,fusion_dim=10)
    ...
    train_audio,train_audio_length=A_V_data(path_train,maxlen=50)
    train_audio=torch.tensor(train_audio)
    train_audio_mask=length_mask(train_audio_length,maxlen=50)
    valid_audio,v_length=A_V_data(path_dev,maxlen=50)
    valid_audio=torch.tensor(valid_audio)
    valid_audio_mask=length_mask(v_length,maxlen=50)
    test_audio,test_length=A_V_data(path_test,maxlen=50)
    test_audio=torch.tensor(test_audio)
    test_audio_mask=length_mask(test_length,maxlen=50)
    train_vision,train_vision_length=A_V_data(path_train,maxlen=50,mode='vision')
    valid_vision, valid_vision_length = A_V_data(path_dev, maxlen=50,mode='vision')
    test_vision, test_vision_length = A_V_data(path_test, maxlen=50, mode='vision')
    max_acc = 0
    min_loss = 100
    if args.do_train:
        ...
        all_train_audio_mask=torch.tensor(train_audio_mask,dtype=torch.float)
        ...
        train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_train_audio,all_train_vision,all_train_audio_mask, all_label_ids)
        ...
        all_valid_audio_mask=torch.tensor(valid_audio_mask,dtype=torch.float32)
        ...
        eval_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_valid_audio,all_valid_vision,all_valid_audio_mask, all_label_ids)
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            with tqdm(train_dataloader, desc="Iteration") as tepoch:
                for step, batch in enumerate(tepoch):
                    ...
                    input_ids, input_mask, segment_ids, all_train_audio,all_train_vision,all_train_audio_mask, label_ids = batch
                    loss, logits = model(input_ids, all_train_audio,all_train_vision, segment_ids, input_mask,all_train_audio_mask, label_ids)
                    if n_gpu > 1:
            ...
            for input_ids, input_mask, segment_ids, all_valid_audio, all_valid_vision,all_valid_audio_mask,label_ids in tqdm(eval_dataloader,
                                                                                       desc="Evaluating"):
                ...
                with torch.no_grad():
                    tmp_eval_loss, logits = model(input_ids, all_valid_audio, all_valid_vision,segment_ids, input_mask,all_valid_audio_mask, label_ids)
            ...
            all_test_audio_mask = torch.tensor(test_audio_mask, dtype=torch.float32)
            ...
            test_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_test_audio,
                                      all_test_vision, all_test_audio_mask)
            # Run prediction for full data
            ...
            with torch.no_grad():
                for input_ids, input_mask, segment_ids, label_ids, all_test_audio, all_test_vision, all_test_audio_mask in tqdm(
                        test_dataloader,
                        desc="Evaluating"):
                    input_ids = input_ids.to(device)
                    input_mask = input_mask.to(device)
    ...
    if args.do_test:
        ...
        all_segment_ids = torch.tensor([f.segment_ids for f in test_features], dtype=torch.long)
        all_label_ids = torch.tensor([f.label_id for f in test_features], dtype=torch.float32)
        test_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_test_audio,all_test_vision,all_test_audio_mask)
        # Run prediction for full data
        test_sampler = SequentialSampler(test_data)
        test_dataloader = DataLoader(test_data, sampler=test_sampler, batch_size=args.test_batch_size)
        '''model=Bert_LMF_Tranformer(hidden_dropout_prob=0.1,hidden_size=768,num_labels=1,fusion_dim=10)
        ...
        fusion_attention_list = []
        with torch.no_grad():
            for input_ids, input_mask, segment_ids, label_ids, all_test_audio,all_test_vision,all_test_audio_mask in tqdm(test_dataloader,
                                                                                      desc="Evaluating"):
                input_ids = input_ids.to(device)
                input_mask = input_mask.to(device)
                segment_ids = segment_ids.to(device)
                ...
                all_test_vision=all_test_vision.to(device)
                with torch.no_grad():
                    tmp_test_loss, logits = model(input_ids, all_test_audio,all_test_vision, segment_ids, input_mask,all_test_audio_mask, label_ids)
                    logits, text_attention, fusion_attention = model(input_ids, all_test_audio,all_test_vision, segment_ids, input_mask,all_test_audio_mask)
        ...
        print(class_report)
        acc = accuracy_score(truth_list1, predict_list1)
        confusion = confusion_matrix(truth_list1, predict_list1)
        corr = numpy.corrcoef(predict_list, truth_list)[0][1]
        mae = numpy.mean(numpy.absolute(predict_list - truth_list))
        loss = tr_loss / nb_tr_steps if args.do_train else None
        results = {'test_loss': test_loss,
...
if __name__ == '__main__':
    ...
    for i in range(10):
        ...
        os.system('rm -r Cross-Modal-BERT-master/CM-BERT_output_three')
